...
JEdit::JEdit(SHORT x, SHORT y, SHORT width, SHORT height, const std::string& text /* = std::string() */, JWindow* parent /* = jApp->Root() */, EchoMode mode /* = JEdit::EM_NORMAL */, VALIDATOR validator /* = DefaultValidator */)
...
void JEdit::OnKeyEvent(JEvent* e)
{
	...
	default:
		if(key >0 && key < 256 && isprint(key) && static_cast<SHORT>(text_.length()) < Width()-1 && validator_(text_, (char)e->GetEventCode()))
		{
